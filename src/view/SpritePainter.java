...
public class SpritePainter {
     ...
     * 4. 绘制道具 (支持动画效果)
     */
    public void drawItem(GraphicsContext gc, item.Item item) {
        if (item == null || !item.isActive() || !item.isVisible()) return;
        ...
        switch (item.getType()) {
            case HEAL:
                itemImage = resourceManager.loadImage("images/lives.png");
                break;
            case INVINCIBLE:
                itemImage = resourceManager.loadImage("images/buff.png");
                break;
            case BOMB:
                itemImage = resourceManager.loadImage("images/bullet.png");
                break;
        }
        ...
        if (itemImage != null) {
            ...
            gc.fillRect(x + offsetX, y + offsetY, scaledWidth, scaledHeight);

            // 绘制道具名称
            gc.setFill(Color.WHITE);
            gc.setFont(javafx.scene.text.Font.font("Arial", 12));
            String text = item.getType().getName().substring(0, 1);
            gc.fillText(text, x + width/2 - 5, y + height/2 + 5);
        }
    ...
    private Color getItemColor(item.ItemType type) {
        switch (type) {
            case HEAL: return Color.RED;
            case INVINCIBLE: return Color.GOLD;
            case BOMB: return Color.DARKRED;
